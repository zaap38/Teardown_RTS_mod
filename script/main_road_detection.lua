...
function exportToRegistry(md)

	local root = "level.mappingData"

	--SetString(root, tableToString(root, md, 900))
	local t = {
		a = 0,
		b = {
			c = {
	...
	local str = tableToString(md)
	--DebugWatch("before", str)
	SetString(root, str)
	--DebugPrint(table.ToString(t, "", false))
	--DebugPrint(tostring(t))
end

function tableToString(t, key, maxDepth)
	key = key or ""
	...
	for k, v in pairs(t) do
		...
		if type(v) == 'table' then
			str = str .. tableToString(v, newKey, maxDepth - 1)
		else
			str = str .. tostring(newKey) .. '=' .. tostring(v) .. ";"
		end
	end
	--DebugPrint(str)
	return str
...
function importFromRegistry()
	local t = {}
	local str = GetString("level.mappingData")
	t = stringToTable(str)
	DebugWatch("after", tableToString(t))
end

function hasKeys(tab, keys)
	local ref = {}
	ref[#ref + 1] = tab
	for i=1, #keys do
		if ref[#ref][keys[i]] ~= nil then
			ref[#ref + 1] = ref[#ref][keys[i]]
		else
			return false
		end
	end
...
function setTableValue(tab, keys, value)
	local disp = ""
	for i=1, #keys do
		disp = disp .. "," .. keys[i]
	end
	...
	ref[#ref] = deepcopy(value)
end

function subTable(t, keys)
	local ct = t
	for i=1, #keys do
		if ct[keys[i]] ~= nil then
			ct = ct[keys]
		end
	end
	return ct
...
function stringToTableOld(str)

	local t = {}
	local split = splitString(str, ';')

	for i=1, #split do
		local splitKeyValue = splitString(split[i], '=')
		local keys = splitKeyValue[1]

		keys = splitString(keys, '.')
		local v = splitKeyValue[2]
		local added = {}
		for j=1, #keys do
			added[#added + 1] = keys[j]
			if not hasKeys(t, added) then
				setTableValue(t, added, {})
			end
		end
		setTableValue(t, added, v)
	end
